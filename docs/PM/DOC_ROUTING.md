...
- 不管本轮改动大还是小，只要文档内容、职责分工、入口关系发生变化，就要在同一轮顺手把本路由表修正到位。
...
- 文档路由维护与文档内容维护同等重要；不能因为“只是小改动”就只改正文、不改入口。
- 如果新增了一份未来会反复使用的专项文档，要判断：
  1. 它服务于哪类任务？
  2. 应该挂到哪个任务类型的默认路径里？
  3. 它属于高层入口，还是下层专项文档？
- 如果某份旧文档已经不再起指导作用，也要考虑从本路由表中降级或移除。
- 如果本轮修改影响到了高层文档如何继续下钻，也要同步回查：
  - [../../AGENTS.md](../../AGENTS.md)
  - [./CODEX_PLAYBOOK.md](./CODEX_PLAYBOOK.md)
  - [./DOC_MAINTENANCE.md](./DOC_MAINTENANCE.md)
  - 必要时 [./SESSION_SUMMARY.md](./SESSION_SUMMARY.md)
