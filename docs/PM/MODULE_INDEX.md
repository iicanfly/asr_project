...
- 角色：实时转写与上传转写共用的 ASR 服务层。
- 主要职责：
  - 统一封装内网 / 外网 ASR 调用分流。
  - 提供 `RealtimeChunkPolicy` 与 `decide_chunk_processing()`，集中管理实时切片触发规则。
  - 提供 `detect_silence()`、`add_wav_header()`、`should_filter_asr_result()` 等可单测的纯函数。
  - 作为后续静音过滤、弱语音处理、双层转写策略的主要扩展落点。

### `tests/test_asr_service.py`
- 角色：Sprint 0 新增的基础自动化测试入口。
- 主要职责：
  - 回归静音检测规则。
  - 回归实时切片触发规则。
  - 回归语气词过滤规则。
- 说明：
  - 目前采用 `unittest`，尽量不依赖完整运行环境，适合先做轻量验证。
