...
| 分段策略 | 连续表达不要切得过碎，明显话题切换要能分开 | 重点看“既不断句过碎，也不整段堆死” |
| 说话人区分 | 多人音频能按说话人拆分 | 第一阶段先做到 `Speaker_1 / Speaker_2...` |
...
## 2026-05-11 / 说话人区分专项补充

### 手工补充检查
- 双人或多人连续说话时：
  - 前端应能看到不同 `Speaker_x` 标签
  - 同一说话人的连续表达不要被频繁错误切换标签
- 如果一个人说完，另一人接着说：
  - 应尽量出现新的说话人标签，而不是继续并到上一人名下
- 如果暂时无法判定说话人：
  - 应有可接受的兜底策略，不能直接把整段结果丢失

### 第一阶段验收口径
- 先验收“能区分不同说话人”，不要求第一阶段就绑定真实姓名
- 先保证说话人区分不会破坏现有实时转写主链路、双环境调用与前端展示
...
- 静音超时自动高级回写：
  - 用户说完后不点击停止录音，仅保持静音。
  - 静音超过约 10 秒后，尾部残留的橙色 / 蓝色文本应自动收口为黑色。
  - 自动高级回写发生后，如果用户继续说话，应只给新增尾部重新着色，不应破坏前面已黑化内容。

### 自动化补充检查
- `tests/test_realtime_tiered_rewrite.py::test_idle_timeout_triggers_high_rewrite_without_stop`
  - 验证静音超时后，不依赖 `stop_recording` 也会触发一次 `high_rewrite`。
  - 验证触发原因字段为 `idle_high_rewrite_timeout`。
...
### 2026-05-10 / 回写兜底与独立“是”回归补充
- 自动化补充检查：
  - `tests/test_asr_service.py` 需要覆盖：
    - 独立片段 `"是"` 会被删除；
    - 正常句内 `“会议目的是明确范围”` 不会被误删。
- 手工补充检查：
  - 真实录音中若出现 `“嗯。是。重要约束。”` 一类结果，前端应直接显示为 `“重要约束”`。
  - 打开浏览器控制台，观察是否出现：
...
## 2026-05-11 / 颜色分层回归补充
- 手工重点检查：
  - 当一段文本已经出现黑色 high 区域后，继续说话触发新的 partial，黑色区域不应被刷成橙色。
  - 当一段文本已经出现蓝色 medium 区域后，继续说话触发新的 partial，蓝色区域不应被刷成橙色；只有新增尾巴应为橙色。
  - 整段 stop 后如果完成 high 收尾，整段最终应收敛为黑色。
