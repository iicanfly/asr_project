...
### 新增必看观察点
- 高回写后分段：
  - 当一次 `high_rewrite` 完成后，如果稳定文本已经约 100 字并且像完整句子，应在这里结束当前段。
  - 分段后再次说话时，应创建新的 `segment_id` 并新开下一条消息。
  - 如果高回写后的文本虽然够长，但句子明显还没收住，则不应强行切段。
- 三级回写颜色：
  - `segment_partial`：深橙色
  - `medium_rewrite`：青绿色
  - `high_rewrite`：黑色
...
- 手工重点检查：
  ...
  - 当一段文本已经出现青绿色 medium 区域后，继续说话触发新的 partial，青绿色区域不应被刷成橙色；只有新增尾巴应为橙色。
