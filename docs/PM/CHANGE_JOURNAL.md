...
- 主题：
  - 根据用户最新判断，重新收敛实时转写主线优先级，并把说话人区分纳入新增重点。
- 修改内容：
  - 更新 `docs/PM/BACKLOG.md`，把主线优先级改为：静音过滤 / 弱语音门控、分段策略、说话人区分。
  - 更新 `docs/PM/SESSION_SUMMARY.md`，同步记录当前主线已从“继续磨三级回写”转向“门控 + 分段 + speaker diarization”。
  - 更新 `docs/PM/TEST_MATRIX.md`，加入说话人区分测试口径，并下调语气词过滤 / 回写等项的优先级。
- 目的：
  - 让后续开发与测试不再继续围绕已经基本过线的能力打转，而是集中火力推进当前最缺的模块。
- 验证方式：
  - `conda run --no-capture-output -n asr python tools/check_doc_corruption.py`
- 当前结果：
  - 文档层面的当前主线已经完成重排，后续可以直接按新优先级推进实现。
- 用户反馈：
  - 用户明确表示：静音过滤 / 弱语音门控、分段是实时转写当前还要继续推进的重点；新增的重要方向是声纹识别，用于按说话人区分语音。
- 后续动作：
  - 下一轮开始先做“说话人区分到底先做 diarization 还是实名绑定”的实现边界确认；默认先按 diarization MVP 推进。

### 2026-05-11 / Commit 待提交
- 主题：
  - 把“每次修改都要同步维护 md 内容与 md 路由”固化为高优先级长期准则。
- 修改内容：
  - 更新 `AGENTS.md`，明确每轮提交前都要检查 Markdown 内容与文档路由是否需要同步维护。
  - 更新 `docs/PM/CODEX_PLAYBOOK.md`，把文档内容维护与文档路由维护都写成提交前默认动作。
  - 更新 `docs/PM/DOC_MAINTENANCE.md`，把 `DOC_ROUTING.md` 与上层入口文档纳入默认检查顺序。
  - 更新 `docs/PM/DOC_ROUTING.md`，强调路由表本身也必须随每轮有效改动同步维护。
  - 更新 `docs/PM/SESSION_SUMMARY.md`，记录这条新固化的长期协作规则。
- 目的：
  - 防止未来出现“代码变了、文档正文变了，但文档入口和阅读路径没跟着变”的失配。
- 验证方式：
  - `conda run --no-capture-output -n asr python tools/check_doc_corruption.py`
- 当前结果：
  - 这条规则已经被写入仓库级记忆文件与 PM 文档体系。
- 用户反馈：
  - 用户明确要求：无论改动大小，都要把 md 内容维护与 md 路由维护视为非常重要的默认准则。
- 后续动作：
  - 本地提交后按用户要求 push 远程。
