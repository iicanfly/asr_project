...
- 当前补充：
  - 如果浏览器本地存储空间不足，当前版本会降级成缓存告警，而不是直接抛异常打断页面逻辑
- 上游调用方：
  - `stopRecording()`
...
- 当前补充：
  - 底层实际通过安全写入封装执行；如果写入失败，会把问题降级成“缓存受限”提示
...
- 当前补充：
  - 如果缓存 JSON 已损坏，当前版本会自动忽略并清理坏缓存，避免恢复流程直接报错
  - 当前版本还会同步恢复消息 ID 计数器，避免后续新消息与旧消息撞 ID
