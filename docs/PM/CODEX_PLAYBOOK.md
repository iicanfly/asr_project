...
- 文档维护不是附属工作：每次修改，不管大改还是小改，在提交前都要默认检查 `.md` 内容是否过期、文档路由是否需要同步改。
- 提交前默认先运行 `python tools/codex_guard.py`。
- 仓库级 `pre-commit` hook 会自动运行 `python tools/codex_guard.py --staged`，避免把“要不要更新文档、文档是否写坏”完全交给记忆。
- 如果实现改动确认不需要更新 Markdown，必须显式使用 `CODEX_ALLOW_NO_DOC_UPDATE=1` 再提交，并在交付说明里写明理由。
...
## 文档维护默认行为
- 这是一条高优先级默认动作：**每轮改动在提交前，都要顺手检查“文档内容”与“文档路由”两件事**，不能因为改动小就跳过。
...
- 如果本轮改变了文档的职责、入口、阅读顺序、默认下钻路径，必须同步维护：
...
- 默认把以下文件视为“需要主动回看”的长期记忆文件：
  - `AGENTS.md`
  - `docs/PM/CODEX_PLAYBOOK.md`
  - `docs/PM/DOC_ROUTING.md`（先判断任务类型，再决定继续下钻哪些专项文档）
...
- 如果要看“更完整、可复用、可长期维护”的任务分流总表，继续下钻到：
  - `docs/PM/DOC_ROUTING.md`
