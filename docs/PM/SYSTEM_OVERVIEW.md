...
## 2026-05-10 / Sprint 0 结构更新
- Python 后端层新增了一个明确的“ASR 服务层”：
  - `main.py` 继续作为 HTTP / Socket.IO 入口。
  - `services/asr_service.py` 负责：
    - ASR 调用分流
    - WAV 封装
    - 静音检测
    - 实时切片决策
    - 基础结果过滤
- 这意味着后续做实时转写质量优化时，优先应改 `services/asr_service.py` 与 `static/js/app.js`，而不是继续把逻辑堆回 `main.py`。
- Web 前端层的实时展示链路也发生了一个重要修正：
  - `handleASRResult()` 现在开始区分“追加新消息”和“替换既有消息”的处理路径。
  - 这为后续实现双层转写回写创造了前置条件。
