...
- 仓库级 `pre-commit` hook 现在会自动运行：
  - `python tools/codex_guard.py --staged`
- 如果实现改动确认不需要更新任何 Markdown，必须显式使用：
  - `CODEX_ALLOW_NO_DOC_UPDATE=1`
  - 并在最终回复里说明为什么这轮文档可不更新。

## 推荐的单轮工作流
1. 完成用户提出的修改。
2. 运行 `python tools/codex_guard.py`，先过提交前闸门。
3. 运行 `git status` 并检查差异。
4. 将本轮应提交的改动做成本地 commit。
5. 只有在用户明确要求远程更新时，才执行 push。
