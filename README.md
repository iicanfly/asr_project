...
- **高级回写后分段**
  - 当一次 `high rewrite` 完成后，如果当前稳定文本已经约 100 字，并且**末尾带有明确句末标点**（如 `。！？；`），且最后一句本身足够完整，不是很短的引导尾句，就在这里结束当前段
  - 后续再次说话时会自动创建新的 `segment_id`，避免已经完成高级回写的旧正文继续被后续语音拖着累计
...
- **空闲收尾**
  - `IDLE_SEGMENT_SPLIT_SECONDS` 默认关闭，不再依赖噪声环境下不稳定的空闲分段
  - 用户说完后如果长时间不继续说话，仍可由 `IDLE_HIGH_REWRITE_SECONDS=10` 触发一次兼容型 `high rewrite` 收尾
  - `IDLE_HIGH_REWRITE_SECONDS=10` 仍保留为更长空闲场景下的兼容收尾阈值
